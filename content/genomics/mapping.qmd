...
As a last tip, try to make use of absolute paths to all important files and folders at the top of your scripts. This makes it easy to change them at a later time, although it is not ideal for making your scripts portable.^[An alternative is to make use of relative paths, but in that case the script still needs to be called from a fixed location in your project folders (or you need to make use of [this strategy](https://stackoverflow.com/a/246128)). For more complex scripts, you might want to look into providing paths as an argument, or to make use of configuration or samplesheets (as is done by more complex workflow manager tools).]
...
A few other important options are:

- `-t <int>`: the number of threads to use. See @tip-parallel.
- `-k <int>`: minimum seed length (used for initial exact matching before extending the alignment) <!-- TODO bwa options-->
- `-R "@RG\tID:L001\tPL:ILLUMINA\tSM:${sample}"`: defines _read groups_, which are tags to identify which reads belong together across flowcells/lanes/library preparations[^read-groups]

[^read-groups]: Reads groups are a [confusing subject](https://www.biostars.org/p/43897/), and [a very deep](https://gatk.broadinstitute.org/hc/en-us/articles/360035532352-Errors-about-read-group-RG-information) one as well. In a nutshell, read groups are used to inform downstream tools about which reads were sequenced together in the same flowcell/lane (to account for technical variability) and which reads need to be grouped because they are derived from the same sample (but stored in separate files). Fortunately, they are only required by a few downstream GATK tools, but leaving them out [can result in errors](https://gatk.broadinstitute.org/hc/en-us/articles/360035532352-Errors-about-read-group-RG-information).
...
bwa mem read_1.fastq read_2.fastq reference.fasta –o alignment.sam
Open the resulting sam file using less.
Align the same file, but immediately pipe the bwa output to samtools for sorting and converting to a .bam file (bwa mem | samtools sort -).
Compare the size of the .sam and .bam files (du -sh).

Inspect and run the map.sh script to map all the reads.
Check the bwa help file to interpret the different options.
-R "@RG\tID:001\tSM:001\tPL:ILLUMINA“ => needed for downstream GATK analysis
What does the samtools flagstat command do?
...
When analysing WGS data (but not AmpliSeq) it is important to remove human reads from your FASTQ files prior to mapping them to the _Plasmodium_ reference genome. There are a number of different approaches and dedicated tools to this - assigning reads to different organisms is a crucial step in bacterial metagenomics for example - but one of the most common approaches for malaria genomics is to simply do a two-step mapping process with your alignment software of choice: in the first round all reads are mapped against the human reference genome. Anything that _"sticks"_ is discarded. In the second round, the remaining unmapped reads are mapped to _Plasmodium_.
...
You could try this for yourself as a bonus exercise.
...
<!-- 3.  You must sort the BAM files from the original ordering produced by `bwa mem` into a *coordinate-ordered* format, in which the reads are sorted by where they appear within the reference genome. -->
...
```bash
bwa mem <arguments> | samtools sort <arguments>
```
